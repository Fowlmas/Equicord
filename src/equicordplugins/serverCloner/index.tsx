...
async function cloneServer(
    ...
    log({ text: `Cloning "${sourceGuild.name}" → target server...`, type: "info" });
    ...
    if (options.embeds && options.channels && !isCancelled()) {
        try {
            ...
            for (const [sourceChId, targetChId] of channelMapping.entries()) {
                ...
                if (sourceCh?.type === 4) continue; // categories don't have messages
...
function ServerClonerUI({ initialSourceId = "" }: { initialSourceId?: string; }) {
    ...
    return (
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div>
                <Forms.FormTitle>Source server</Forms.FormTitle>
                <Select
                    options={allGuilds}
            ...
            <div>
                <Forms.FormTitle>Target server (ADMIN required)</Forms.FormTitle>
            ...
            <div>
                <Forms.FormTitle>Cloning options</Forms.FormTitle>
            ...
            <div style={{ display: "flex", gap: 8 }}>
                <Button
                    ...
                    color={running ? Button.Colors.PRIMARY : Button.Colors.BRAND}
...
function ServerClonerModal({ rootProps, guildId }: { rootProps: any; guildId: string; }) {
    return (
        <Modal {...rootProps} title="Server Cloner" size="lg">
