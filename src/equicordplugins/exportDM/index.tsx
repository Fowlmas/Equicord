...
const AuthenticationStore = findByPropsLazy("getToken");
...
function getToken(): string {
    try {
        return AuthenticationStore?.getToken?.() ?? "";
    } catch { return ""; }
}
...
async function fetchAllMessages(channelId: string, token: string, onProgress: (n: number) => void): Promise<RichMessage[]> {
    ...
    try {
        const cached = MessageStore.getMessages?.(channelId) as any;
        if (cached) {
...
function buildTxt(messages: RichMessage[], channelName: string): string {
    const lines = [`=== Export DMs — ${channelName} ===`, `Exported on ${new Date().toLocaleString()}`, `Total: ${messages.length} messages`, ""];
    for (const m of messages) {
        ...
        lines.push(`[${d}]${m.editedAt ? " (edited)" : ""} ${m.authorName}: ${m.content}`);
...
function buildMd(messages: RichMessage[], channelName: string): string {
    const lines = [`# Export DMs — ${channelName}`, `> Exported on ${new Date().toLocaleString()} · **${messages.length} messages**`, ""];
    for (const m of messages) {
        const d = new Date(m.timestamp).toLocaleString();
        if (m.referencedMessage) lines.push(`> **${m.referencedMessage.authorName}**: ${m.referencedMessage.content}`);
        lines.push(`**${m.authorName}** — *${d}*${m.editedAt ? " (edited)" : ""}${m.pinned ? " [pinned]" : ""}`);
...
function buildHtml(messages: RichMessage[], channelName: string): string {
    const rows = messages.map(m => {
        ...
        const mediaHtml = m.attachments.map(a => {
            const mediaType = getMediaType(a.url, a.contentType);
            if (mediaType === "image") return `<div class="media"><img src="${a.url}" alt="${a.filename}" loading="lazy"><div class="media-name">${a.filename} (${formatSize(a.size)})</div></div>`;
            if (mediaType === "video") return `<div class="media"><video src="${a.url}" controls preload="none"></video><div class="media-name">${a.filename}</div></div>`;
            if (mediaType === "audio") return `<div class="media"><audio src="${a.url}" controls></audio><div class="media-name">${a.filename}</div></div>`;
...
function ExportDMModal({ rootProps }: { rootProps: any; }) {
    ...
    async function doExport() {
        ...
        if (!token) { setStatus("error"); setProgress("Token not found"); return; }
        ...
        for (let i = 0; i < sel.length; i++) {
            ...
            let msgs = await fetchAllMessages(ch.id, token, n => setProgress(`${prefix}Fetching: ${n} messages...`));
            ...
            setProgress(`${prefix}${msgs.length} messages — generating file...`);
        ...
        setStatus("done");
        setProgress(`${selected.size} conversations exported`);
    }
    ...
    const FORMATS: Array<{ key: ExportFormat; label: string; desc: string; }> = [
        { key: "html", label: "HTML", desc: "Web page with images/videos" },
        { key: "txt", label: "TXT", desc: "Plain text" },
        { key: "md", label: "MD", desc: "Markdown" },
        { key: "json", label: "JSON", desc: "Full structured" },
    ...
    return (
        <Modal {...rootProps} title="Export DMs" size="md">
            <div className="edm-content">
                ...
                <div className="edm-search-bar">
                    ...
                    <input
                        ...
                        type="text"
                        placeholder="Search for a friend or a DM..."
                        value={search}
                ...
                <Forms.FormTitle tag="h5" className="edm-label">Choose a conversation</Forms.FormTitle>
                <div className="edm-channel-list">
                    {filtered.length === 0 && (
                        <div className="edm-empty">
                            {search ? `No results for "${search}"` : "No DMs found"}
                        </div>
                    )}
                    {filtered.map(c => {
                ...
                <Forms.FormTitle tag="h5" className="edm-label">Export format</Forms.FormTitle>
