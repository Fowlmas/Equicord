...
const DS_KEY = "equicord-followme-target";
...
const ctxPatch: NavContextMenuPatchCallback = (children, props) => {
    ...
    try {
        ...
        const followMeIcon = () => <FollowMeIcon filled={isFollowed} />;

        children.push(
            <Menu.MenuItem
                id="follow-me-ctx"
                label={isFollowed ? "Stop Follow Me" : "Follow Me"}
                color={isFollowed ? "danger" : undefined}
                action={() => { if (isFollowed) unfollowMe(); else followMe(userId); }}
                icon={followMeIcon}
                leadingAccessory={{ type: "icon", icon: followMeIcon }}
            />
        );
    } catch (e) {
